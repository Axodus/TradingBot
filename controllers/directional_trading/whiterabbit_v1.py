...
class WhiteRabbitV1Controller(DirectionalTradingControllerBase):
    def __init__(self, config: WhiteRabbitV1ControllerConfig, *args, **kwargs):
        self.config = config
        self.max_records = max(config.bb_length, config.bb_std, config.rsi_length, config.rsi_oversold, config.rsi_overbougth)
    ...
    async def update_processed_data(self):
        ...
        # Generate signal Long and short conditions using OR (|) and AND (&) operators
