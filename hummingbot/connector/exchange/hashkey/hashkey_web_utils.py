...
def rest_url(path_url: str, domain: str = CONSTANTS.DEFAULT_DOMAIN) -> str:
    ...
    :param domain: the Hashkey domain to connect to ("mainnet" or "testnet"). The default value is "mainnet"
        ...
        if response.status != 200:
            if return_err:
                ...
                if error_response is not None and "ret_code" in error_response and "ret_msg" in error_response:
                    raise IOError(f"The request to Hashkey failed. Error: {error_response}. Request: {request}")
                else:
